...
class FeatureLoss(nn.Module):
    def __init__(self, loss, blocks, weights, device):
        ...
        import os
        local_w = os.environ.get("VGG16BN_FEATURES_WEIGHTS")
        if local_w and os.path.exists(local_w):
            # offline environment: load torchvision vgg16_bn feature weights
            # from a local file instead of download.pytorch.org
            vgg = vgg16_bn(weights=None).features
            vgg.load_state_dict(torch.load(local_w, map_location="cpu",
