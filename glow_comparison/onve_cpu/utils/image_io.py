...
def crop_image(img, d=32):
    ...
    target_size = (img_cropped.size[0], img_cropped.size[1])  # 新的宽度和高度
    resize_img = img.resize(target_size)
    # print("----------------after crop-------------------")
    # print(resize_img.size[0])
    # print(resize_img.size[1])
    return resize_img, img.size[0], img.size[1]
...
def save_fusion(image, path, name):
    b = image.size()[0]
    for i in range(b):
        t = image.data[i]
        t[t > 1] = 1
...
def save_graph(name, graph_list, output_path="output_segmentation/"):
    plt.clf()
    plt.plot(graph_list)
...
def prepare_image(file_name):
    ...
    """
    img_pil, w, h = crop_image(get_image(file_name, -1)[0], d=32)
    return pil_to_np(img_pil), w, h
